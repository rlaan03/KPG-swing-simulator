...
with cols[0]:
    ...
    # 여기서 clicks 정의!
    clicks = plotly_events(fig, click_event=True, hover_event=False, override_height=850, key="graph")
        ...
        # 3) 시뮬 & 노드 선택 모두 된 경우에만 스크롤 영역에 그래프 삽입
            ...
            for b in st.session_state.selected_buses:
                if b in res["buses"]:
                    ...
                    """
            slot_html += "</div>"
            components.html(slot_html, height=850)
...
if st.session_state.sim_done and "sim_res" in st.session_state:
    ...
    with col2:
        ...
        fig.update_yaxes(range=[34, 38.2], scaleanchor="x", scaleratio=1)
